 ...
 * submit管理
 */
KISSY.add('iee/submiturl', function(S, DOM, Event, IO, Modal){
    ...
    Module.renderPager = function(data){
        ...
        if(1 < size){
            ...
            html += data.prev ?  '<a title=上一页 href="/my/query?action=submiturl&&page=prev&id=' + firstID + '" class="pager-prev" data-act="submiturl/prev:' + firstID + '"></a>' : '<span class="pager-prev"></span>';
            html += data.next ? '<a title=下一页 href="/my/query?action=submiturl&page=next&id=' + lastID + '" class="pager-next" data-act="submiturl/next:' + lastID + '"></a>' : '<span class="pager-next"></span>';
    ...
    Module.renderList = function(data){
        ...
        if(data.size){
            S.each(data.list, function(vo){
                ...
                if('s' === vo.status){
                    html += '将状态置为:<a class="btn" href="/my/status?action=submiturl&field=p&id=' + id + '" data-act="submiturl/status">通过</a>';
                    html += '<a class="btn" href="/my/postnew" data-act="submiturl/compose:' + id + '">发表文章</a>';
                    html += '<a class="btn" href="/my/status?action=submiturl&field=remove&id=' + id + '" data-act="submiturl/status">删除</a>';
                }else{
